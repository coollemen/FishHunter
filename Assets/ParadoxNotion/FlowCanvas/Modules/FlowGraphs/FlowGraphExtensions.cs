...
namespace FlowCanvas
{
    ...
    public static class FlowGraphExtensions
    {
        ...
        public static UnityEditor.GenericMenu GetFullNodesMenu(this FlowGraph flowGraph, Vector2 mousePos, Port context, Object dropInstance)
        {
            ...
            {
                menu = flowGraph.AppendTypeReflectionNodesMenu(menu, context.type, "", mousePos, context, dropInstance);
                menu = flowGraph.AppendAllReflectionNodesMenuExcludeSelectType(menu, context.type, "Functions/Reflected", mousePos, context, dropInstance);
            }
            else
            {
        ...
        public static UnityEditor.GenericMenu AppendSimplexNodesMenu(this FlowGraph graph, UnityEditor.GenericMenu menu, string baseCategory, Vector2 pos, Port sourcePort, object dropInstance)
        {
            ...
            foreach (var _info in infos)
            {
                ...
                if (sourcePort != null)
                {
                    ...
                    //SL--
                    bool isCallableFunction = info.type.RTIsSubclassOf(typeof(CallableFunctionNodeBase));
                    ...
                    if (method != null)
                    {
                        ...
                        {
                            //SL--------------
                            if (!isCallableFunction && method.ReturnType != typeof(void) && method.ReturnType != typeof(System.Collections.IEnumerator))
                            {
                                continue;
                            }
                ...
                //SL--
                //var category = string.Join("/", new string[] { baseCategory, info.category, info.name }).TrimStart('/');
                var category = string.Join("/", new string[] { !info.category.Contains("UnityEngine/") ? baseCategory : "", info.category, info.name }).TrimStart('/');
        ...
        //SL--exclude targetType
        public static UnityEditor.GenericMenu AppendAllReflectionNodesMenuExcludeSelectType(this FlowGraph graph, UnityEditor.GenericMenu menu, System.Type exculdeType, string baseCategory, Vector2 pos, Port sourcePort, object dropInstance)
        {
            foreach (var type in TypePrefs.GetPreferedTypesList())
            {
                if (exculdeType != type)
        ...
        public static UnityEditor.GenericMenu AppendMacroNodesMenu(this FlowGraph graph, UnityEditor.GenericMenu menu, string baseCategory, Vector2 pos, Port sourcePort, object dropInstance)
        {
            ...
            foreach (var guid in projectMacroGUIDS)
            {
                ...
                //SL-- groupMacro不显示在列表中，因为黑版用的不是localBlackboard
                if (macro.GetType() != typeof(GroupMacro))
                {
                    if (macro != graph)
                    {
                        ...
                        //SL--
                        if (!string.IsNullOrEmpty(macro.CategoryPath))
                        {
                            if (!macro.CategoryPath.EndsWith("/"))
                            {
                                macro.CategoryPath = macro.CategoryPath + "/";
                            }
                            if (!macro.CategoryPath.StartsWith("/"))
                            {
                                macro.CategoryPath = "/" + macro.CategoryPath;
                            }
                        }
        ...
        public static void ConvertNodesToMacro(List<Node> originalNodes)
        {
            ...
            for (var i = 0; i < originalNodes.Count; i++)
            {
                ...
                foreach (var originalInputConnection in originalNode.inConnections.OfType<BinderConnection>())
                {
                    ...
                    if (defSourcePort.CanAcceptConnections())
                    { //check this for case of merged FlowPorts
                ...
                foreach (var originalOutputConnection in originalNode.outConnections.OfType<BinderConnection>())
                {
                    ...
                    if (defTargetPort.CanAcceptConnections())
                    { //check this for case of merged ValuePorts
            ...
            {
                //SL--
                graph.RemoveNode(node, false, false);
            }
        ...
        ///Convert nodes to macro (with a bit of hocus pocus)
        public static void ConvertNodesGroup(List<Node> originalNodes, Macro newMacro, MacroNodeWrapper wrapper)
        {
            ...
            //undo
            var graph = (FlowScriptBase)originalNodes[0].graph;
            graph.RecordUndo("Convert To GroupMacro");

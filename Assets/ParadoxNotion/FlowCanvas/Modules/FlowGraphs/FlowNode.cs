    ...
    abstract public partial class FlowNode : Node, ISerializationCallbackReceiver
    {
                        ...
                        if (port.willDraw)
                        {
                            ...
                            if (GraphEditorUtility.allowClick)
                            {
                                ...
                                if (e.type == EventType.ContextClick && portRect.Contains(e.mousePosition))
                                {
                                    ...
                                    BinderConnection[] binderConnections = GetInPortConnections(port);
                                    if (port.isConnected)
                                    {
                                        menu.AddItem(new GUIContent("ClearConnects"), false, () =>
                                        {
                                            foreach (var c in binderConnections)
                                            {
                                                graph.RemoveConnection(c);
                                            }
                                            Undo.RecordObject(graph, "ClearConnects");
                                        });
                                    }
                                    if (graph.tempPorts != null && graph.tempPorts.Count > 0)
                                    {
                                        if (port.IsValuePort() && port.isConnected)
                                        {
                                            menu.AddItem(new GUIContent("Replace"), false, () =>
                                            {
                                                if (port.IsFlowPort() && port.IsOutputPort())
                                                {
                                                    var bindConnets = GetOutPortConnections(port);
                                                    graph.RemoveConnection(bindConnets[0]);
                                                }
                                                if (port.IsValuePort() && port.IsInputPort())
                                                {
                                                    var bindConnets = GetInPortConnections(port);
                                                    graph.RemoveConnection(bindConnets[0]);
                                                }
                                                foreach (var p in graph.tempPorts)
                                                {
                                                    Port sourcePort = (Port)p;
                                                    BinderConnection.Create(sourcePort, port);
                                                }
                                                graph.tempPorts = null;
                                                Undo.RecordObject(graph, "Replace");
                                            });
                                        }
                                        if (port.type.IsValueType && !port.isConnected || port.IsFlowPort())
                                        {
                                    ...
                                    {
                                        menu.AddSeparator("");
                                        if (binderConnections.Length != 1)
                                        {
                                            menu.AddItem(new GUIContent("CutAllConnects"), false, () =>
                                            {
                                                List<object> temp = new List<object>();
                                                binderConnections.ForEach((x) => { temp.Add(x.sourcePort); });
                                                ...
                                                Undo.RecordObject(graph, "CutAllConnects");
                                            });
                                        }

                                        if (binderConnections.Length > 0)
                                        {
                                            foreach (var c in binderConnections)
                                            {
                                                menu.AddItem(
                                                    new GUIContent("CutConnect:" +
                                                                   ((c.sourceNode.GetType()
                                                                       .IsSubclassOf(typeof(FlowCanvas.Nodes.VariableNode)))
                                                                       ? c.sourcePort.type.ToString()
                                                                       : string.IsNullOrEmpty(c.sourceNode.name)
                                                                           ? "Dummy_ID:" + c.sourceNode.ID
                                                                           : c.sourceNode.name.Replace("/", "") + "ID:" +
                                                                             c.sourceNode.ID + " Port: " + c.sourcePort.name)),
                                                    false, () =>
                                                    {
                                                        List<object> temp = new List<object>();
                                                        temp.Add(c.sourcePort);
                                                        graph.tempPorts = temp;

                                                        graph.RemoveConnection(c);
                                                        Undo.RecordObject(graph, "CutConnect");
                                                    });
                                            }
                                        }
                                        ...
                                        if (binderConnections.Length != 1)
                                        {
                                            menu.AddItem(new GUIContent("ToggleAllConnectActive"), false, () =>
                                            {
                                                binderConnections.ForEach((x) => x.isActive = !x.isActive);
                                                Undo.RecordObject(graph, "ToggleAllConnectActive");
                                            }
                                            );
                                        }

                                        binderConnections.ForEach(
                                            (x) =>
                                                menu.AddItem(
                                                    new GUIContent("ToggleConnectActive:" +
                                                                   ((x.sourceNode.GetType()
                                                                       .IsSubclassOf(typeof(FlowCanvas.Nodes.VariableNode)))
                                                                       ? x.sourcePort.type.ToString()
                                                                       : string.IsNullOrEmpty(x.sourceNode.name)
                                                                           ? "Dummy_ID:" + x.sourceNode.ID
                                                                           : x.sourceNode.name.Replace("/", "") + "ID:" +
                                                                             x.sourceNode.ID + " Port: " + x.sourcePort.name)),
                                                    false, () =>
                                                    {
                                                        x.isActive = !x.isActive;
                                                        Undo.RecordObject(graph, "ToggleConnectActive");
                                                    }));



                                        #region 将connect 复制到dummy节点

                                        menu.AddSeparator("");

                                        bool portTypeIsFlow = port.type == typeof(Flow); //是否是flow 类型的连线
                                        menu.AddItem(
                                            new GUIContent(portTypeIsFlow
                                                ? "CopyLinkToFlowRemoteDummy"
                                                : "CopyLinkToValueRemoteDummy"), false, () =>
                                                {
                                                    //根据连线类型创建 flow 节点或 value节点
                                                    FlowNode newNode = portTypeIsFlow
                                                            ? graph.AddNode<FlowCanvas.Nodes.Dummy>(port.pos - new Vector2(100f, 0))
                                                            : (FlowNode)
                                                            graph.AddNode(typeof(FlowCanvas.Nodes.ValuePoint<>),
                                                                (port.pos - new Vector2(100f, 0)));

                                                    List<object> temp = new List<object>(); //记录复制的port
                                                    ...
                                                    if (portTypeIsFlow) //将dummy节点的output槽口连接到当前节点槽口
                                                    {
                                                        BinderConnection.Create(newNode.GetOutputPort("Out"), port);
                                                    }

                                                    //
                                                    if (portTypeIsFlow) //将当前节点的输入线,复制到dummy的输入槽口
                                                    {
                                                        foreach (var p in graph.tempPorts)
                                                        {
                                                            Port sourcePort = (Port)p;
                                                            BinderConnection.Create(sourcePort, newNode.GetInputPort("In"));
                                                        }
                                                    }
                                                    else
                                                    {
                                                        var bc = FlowCanvas.BinderConnection.Create((Port)graph.tempPorts[0],
                                                                newNode.GetInputPort("In"));
                                                        //因为的泛类的value dummy, 需要一些trick实现  dummy节点的输入输出槽口连线复制
                                                        FlowCanvas.BinderConnection.Create(
                                                                ((FlowCanvas.FlowNode)(bc.targetNode)).GetOutputPort("Out"), port);
                                                    }

                                                    graph.tempPorts = null; //重置临时记录的槽口

                                                    Undo.RecordObject(graph, "CopyLinkToDummyNode");
                                                });
                                    }
                                    ...
                                    if (menu != null)
                                    {
                                        GraphEditorUtility.PostGUI += () => { menu.ShowAsContext(); }; //Post GUI cause of zoom
                                    }
                    ...
                    for (var i = 0; i < orderedOutputs.Length; i++)
                    {
                        ...
                        if (port.willDraw)
                        {
                            ...
                            if (GraphEditorUtility.allowClick)
                            {
                                ...
                                {
                                    var menu = new GenericMenu();
                                    BinderConnection[] binderConnections = GetOutPortConnections(port);
                                    ...
                                    if (graph.tempPorts != null && graph.tempPorts.Count > 0)
                                    {
                                        //if (port.type == ((Port) graph.tempPorts[0]).type)
                                        //{
                                        if (port.IsFlowPort() && port.isConnected)
                                        {
                                            menu.AddItem(new GUIContent("Replace"), false, () =>
                                            {
                                                if (port.IsFlowPort() && port.IsOutputPort())
                                                ...
                                                {
                                                    Port sourcePort = (Port)p;
                                                    BinderConnection.Create(port, sourcePort);
                                                }
                                                graph.tempPorts = null;
                                                Undo.RecordObject(graph, "Replace");
                                            });
                                        }
                                        if (port.IsFlowPort() && !port.isConnected || port.IsValuePort())
                                        {
                                            menu.AddItem(new GUIContent("Paste"), false, () =>
                                            {
                                                foreach (var p in graph.tempPorts)
                                    ...
                                    if (port.isConnected)
                                    {
                                        ...
                                        if (binderConnections.Length > 0)
                                        {
                                            foreach (var c in binderConnections)
                                            {
                                                menu.AddItem(
                                                    new GUIContent("CutConnect:" +
                                                                   ((c.targetNode.GetType()
                                                                       .IsSubclassOf(typeof(FlowCanvas.Nodes.VariableNode)))
                                                                       ? c.targetPort.type.ToString()
                                                                       : string.IsNullOrEmpty(c.targetNode.name)
                                                                           ? "Dummy_ID:" + c.targetNode.ID
                                                                           : c.targetNode.name.Replace("/", "") + "ID:" + c.targetNode.ID +
                                                                             " Port: " + c.targetPort.name)), false, () =>
                                                                             {
                                                                                 List<object> temp = new List<object>();
                                                                                 temp.Add(c.targetPort);
                                                                                 graph.tempPorts = temp;
                                        ...
                                        if (binderConnections.Length > 0)
                                        {
                                            binderConnections.ForEach(
                                                (x) =>
                                                    menu.AddItem(
                                                        new GUIContent("ToggleConnectActive:" +
                                                                       ((x.targetNode.GetType()
                                                                           .IsSubclassOf(typeof(FlowCanvas.Nodes.VariableNode)))
                                                                           ? x.targetPort.type.ToString()
                                                                           : string.IsNullOrEmpty(x.targetNode.name)
                                                                               ? "Dummy_ID:" + x.targetNode.ID
                                                                               : x.targetNode.name.Replace("/", "") + "ID:" +
                                                                                 x.targetNode.ID + " Port: " + x.targetPort.name)),
                                                        false, () =>
                                                        {
                                                            x.isActive = !x.isActive;
                                                            Undo.RecordObject(graph, "ToggleConnectActive");
                                        ...
                                        bool portTypeIsFlow = port.type == typeof(Flow); //是否是flow 类型的连线
                                        menu.AddItem(
                                            new GUIContent(portTypeIsFlow ? "CopyLinkToFlowRemoteDummy" : "CopyLinkToValueRemoteDummy"), false,
                                            () =>
                                            {
                                                //根据连线类型创建 flow 节点或 value节点
                                                FlowNode newNode = portTypeIsFlow
                                                        ? graph.AddNode<FlowCanvas.Nodes.Dummy>(port.pos + new Vector2(50f, 0))
                                                        : (FlowNode)
                                                        graph.AddNode(typeof(FlowCanvas.Nodes.ValuePoint<>), (port.pos + new Vector2(50f, 0)));
                                                ...
                                                BinderConnection bc;
                                                Port dummyTargetPort;
                                                //将dummy节点的Input槽口连接到当前节点槽口
                                                bc = BinderConnection.Create(port, newNode.GetInputPort("In"));
                                                dummyTargetPort = ((FlowNode)bc.targetNode).GetOutputPort("Out");
                                                //
                                                if (portTypeIsFlow) //将记录的目标port,复制到dummy的输出槽口
                                                {
                                                    BinderConnection.Create(newNode.GetOutputPort("Out"), (Port)graph.tempPorts[0]);
                                                    //flow 类型的连线输出只有一条
                                                }
                                                else //value连线 有多个目标输出槽口
                                                {
                                                    foreach (var tp in graph.tempPorts)
                                                    {
                                                        Port targetPort = (Port)tp;
                                                        BinderConnection.Create(dummyTargetPort, targetPort);
                                                    }
                                                }
                                                GatherPorts();
                                                graph.tempPorts = null; //重置临时记录的槽口
            ...
            if (clickedPort != null && e.type == EventType.MouseUp)
            {
                ...
                else
                {
                    ...
                    if (dragDropMisses == graph.allNodes.Count && clickedPort != null)
                    {
                        ...
                        if (zoomFactor == 1)
                        {
                            menu.ShowAsBrowser(string.Format("Add & Connect (Type of {0})", cachePort.type.FriendlyName()), graph.baseNodeType);
                        }
                        else
                        {
                            if (Event.current.shift)
                            {
                                menu.ShowAsBrowser(new Vector2(rect.x, rect.y) / zoomFactor, string.Format("Add & Connect (Type of {0})", cachePort.type.FriendlyName()), graph.baseNodeType);
                            }
                            else
                            {
        ...
        void DrawNodePort(Port port, Port mirrorOutput = null)
        {
            ...
            if (port.willDraw || (mirrorOutput != null && mirrorOutput.willDraw))
            {
                ...
                if (port.IsInputPort())
                {
                    GUI.color = EditorGUIUtility.isProSkin ? GUI.color : Color.white.WithAlpha(0.6f);
        ...
        protected override GenericMenu OnContextMenu(GenericMenu menu)
        {
            ...
            if (outputPorts.Values.Any(p => p is FlowOutput))
            { //breakpoints only work with FlowOutputs
        ...
        public override void OnDestroy(bool IsReplace)
        {
            if (!IsReplace)
            {
                List<Port> flowinport = new List<Port>();
                List<Port> flowoutport = new List<Port>();
                foreach (var v in inputPorts)
                {
                    if (v.Value.type == typeof(Flow))
                    {
                        if (v.Value.isConnected)
                            flowinport.Add(v.Value);
                    }
                }
                foreach (var v in outputPorts)
                {
                    if (v.Value.type == typeof(Flow))
                    {
                        if (v.Value.isConnected)
                            flowoutport.Add(v.Value);
                    }
                }

                if (flowinport != null && flowinport.Count > 0 && flowoutport != null && flowoutport.Count > 0)
                {
                    BinderConnection[] binderConnections = null;
                    for (int i = 0; i < flowinport.Count; i++)
                    {
                        binderConnections = GetInPortConnections(flowinport[i]);

                        List<object> temp = new List<object>();
                        binderConnections.ForEach((x) => { temp.Add(x.sourcePort); });
                        if (graph.tempPorts == null)
                            graph.tempPorts = new List<object>();
                        graph.tempPorts.AddRange(temp);
                    ...
                    List<Port> targetPorts = new List<Port>();
                    for (int i = 0; i < flowoutport.Count; i++)
                    {
                        binderConnections = GetOutPortConnections(flowoutport[i]);
                        targetPorts.Add(binderConnections[0].targetPort);
                        foreach (var c in binderConnections)
                        {
                            graph.RemoveConnection(c);
                        }
                    ...
                    for (int i = 0; i < graph.tempPorts.Count; i++)
                    {
                        Port sourcePort = (Port)graph.tempPorts[i];
                        //Debug.Log((int)Mathf.Repeat(i, targetPorts.Count));
                        BinderConnection.Create(sourcePort, targetPorts[(int)Mathf.Repeat(i, targetPorts.Count)]);
                    }

...
namespace NodeCanvas.Editor
{
    ...
    public partial class GraphEditor : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            {
                currentGraph = curr;
#if !UNITY_2018_3_0_OR_NEWER
                //SL--强制更新节点引用黑板
                if (currentGraph != null) currentGraph.initialReferencesUpdated = false;
#endif
        ...
        //SL--
        static List<IGraphElement> currentMultiSelection = new List<IGraphElement>();
        //Do graphical multi selection box for nodes
        static void DoCanvasRectSelection(Rect container)
        {

            if (GraphEditorUtility.allowClick && e.type == EventType.MouseDown && e.button == 0 && !e.alt && canvasRect.Contains(CanvasToView(e.mousePosition)))
            {

                currentMultiSelection.Clear();
                if (GraphEditorUtility.activeElements != null && GraphEditorUtility.activeElements.Count > 0)
                    currentMultiSelection.AddRange(GraphEditorUtility.activeElements);
            ...
            if (isMultiSelecting && e.rawType == EventType.MouseUp)
            {
                ...
                if (e.control && e.shift && rect.width > 50 && rect.height > 50)
                {
                ...
                else if (e.control && !e.shift)
                {
                    ...
                    {
                        GraphEditorUtility.activeElements = currentMultiSelection.Union(overlapedNodes.Cast<IGraphElement>().ToList()).ToList();
                        e.Use();
                    }

                    Undo.RegisterCompleteObjectUndo(currentGraph, "Add nodes");
                }
                else if (!e.control && e.shift)
                {
                    if (overlapedNodes.Count > 0)
                    {
                        //Debug.Log("currentnodes:" + currentMultiSelection.Count + "reduce nodes:" + overlapedNodes.Count);
                        GraphEditorUtility.activeElements = currentMultiSelection.Except(overlapedNodes.Cast<IGraphElement>().ToList()).ToList();
                        e.Use();
                    }

                    Undo.RegisterCompleteObjectUndo(currentGraph, "reduce nodes");
                }
                else
                {
                    if (overlapedNodes.Count > 0)
            ...
            if (isMultiSelecting)
            {
                ...
                if (rect.width > 5 && rect.height > 5)
                {
                    ...
                    if (rect.width > 50 && rect.height > 50)
                    {
                        ...
                        GUI.Label(new Rect(e.mousePosition.x + 16, e.mousePosition.y, 120, 22), "<i>ctrl+shift for group</i>", StyleSheet.labelOnCanvas);
                    }
                }
            }
        ...
        static void DoBreadCrumbNavigation(Graph root)
        {
            ...
            //SL--
            var graphInfo = string.Format("<color=#ff4d4d>({0})</color>          Zoom Factor:{1:N1}  ", resultInfo, 1 / zoomFactor);
                    ...
                    if (handleRect.Contains(e.mousePosition))
                    {
                        ...
                        if (e.button == 1)
                        {
                            ...
                            menu.AddItem(new GUIContent("Delete Group"), false, () =>
                            {
                                ...
                                if (tempCanvasGroupNodes.Length > 0 && EditorUtility.DisplayDialog("Delete Group", "Delete contained nodes and subGroup as well?", "Yes", "No"))
                                {
                                    foreach (var node in tempCanvasGroupNodes)
                                    {
                                        currentGraph.RemoveNode(node);
                                    }
                                    foreach (var _group in tempCanvasGroupGroups)
                                    {
                                        currentGraph.canvasGroups.Remove(_group);
                                    }
                                    Undo.RegisterCompleteObjectUndo(currentGraph, "Delete Group Nodes");
                                }
                            });
            ...
            if (Prefs.minimapSize != Prefs.MINIMAP_MIN_SIZE)
            {
                ...
                if (currentGraph.canvasGroups != null)
                {
                    for (var i = 0; i < currentGraph.canvasGroups.Count; i++)
                    {
                        ...
                        //SL--
                        GUI.Label(blipRect, group.name, StyleSheet.miniMapLabel);
                    }
                }
                ...
                if (currentGraph.allNodes != null)
                {
                    ...
                    for (var i = 0; i < currentGraph.allNodes.Count; i++)
                    {
                        ...
                        for (var j = 0; j < node.outConnections.Count; j++)
                        {
                            ...
                            //SL--
                            Handles.color = Application.isPlaying ? StyleSheet.GetStatusColor(connection.status) : connection.defaultColor;

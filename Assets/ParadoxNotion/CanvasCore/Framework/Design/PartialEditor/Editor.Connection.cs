            ...
            if ( GraphEditorUtility.allowClick && e.type == EventType.MouseDown ) {
                ...
                if ( onConnection || onStart || onEnd || onCenter ) {
                    if ( e.button == 0 ) {
                        ...
                        //SL-- 双击插入节点
                        if (typeof(FlowCanvas.BinderConnection).IsAssignableFrom(this.GetType()) && e.clickCount == 2)
                        {
                            graph.InsetNodeBetweenConnect((FlowCanvas.BinderConnection)this, e.mousePosition);
                        }
                    }
                    e.Use();
                }

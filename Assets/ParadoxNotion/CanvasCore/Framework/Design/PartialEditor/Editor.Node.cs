    ...
    partial class Node
    {
        ...
        public Color nodeColor
        {
            get
            {
                ...
                if (!colorLoaded)
                {
                    ...
                    {
                        colorAttributeColor = ColorUtils.HexToColor(NodeColor());
                        hexColor = NodeColor();
                    }
                    else
                    {
        ...
        static void DrawNodeWindow(Node node, Vector2 canvasMousePos, float zoomFactor)
        {
            ...
            else
            {
                ...
                if (node.isSelected)
                {
                    ...
                    GUI.color = new Color(1f, 1f, 0.45f);
                    GUI.Box(node.rect, string.Empty, StyleSheet.windowHighlight);
                }
            }
        ...
        static void ShowHeader(Node node)
        {
            ...
            if (!node.showIcon || node.iconAlignment != Alignment2x2.Default)
            {
                ...
                {
                    //string hex;
                    //var isEditorGUIUtility.isProSkin = EditorGUIUtility.isProSkin;
                    //if (node.nodeColor != default(Color)){
                    //	hex = node.nodeColor.grayscale > 0.6f? DEFAULT_HEX_COLOR_DARK : DEFAULT_HEX_COLOR_LIGHT;
                    //} else {
                    //	hex = isEditorGUIUtility.isProSkin? node.hexColor : DEFAULT_HEX_COLOR_DARK;
                    //}

                    //if (node.nodeColor != default(Color))
                    //{
                    GUI.color = node.nodeColor;
                    var headerHeight = node.rect.height <= 33 ? 33 : 27;
                    ...
                    var text = string.Format("<b><size=12><color=#{0}>{1}</color></size></b>", "FFFFE0", finalTitle);
